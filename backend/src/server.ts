...
app.post("/auth/forgot-password", async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { userId: Number(plain.id), tenantId: Number(plain.companyId || plain.company_id || 0), purpose: "pwdReset" },
...
app.post("/auth/reset-password", async (req, res) => {
  try {
    ...
    const payload = jwt.verify(token, env.JWT_REFRESH_SECRET) as { userId: number; tenantId: number; purpose?: string };
    ...
    try { u.set("passwordHash", hash); } catch {}
    try { (u as any).set?.("password", hash); } catch {}
    if (typeof (u as any).save === "function") {
      await (u as any).save({ fields: ["passwordHash", "password"] });
    } else {
      await (u as any).update({ passwordHash: hash, password: hash });
    }
