...
async function main() {
  ...
  try {
    const r = await db.query('select id, "companyId" from "Users" where email = $1 limit 1', [email]);
    if (r.rows.length === 0) {
      // seed se não existir
    ...
    await db.query('update "Users" set "passwordHash" = $1 where email = $2', [hash, email]);
