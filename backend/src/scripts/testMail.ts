...
async function main() {
  ...
  const url = sanitized + '/auth/forgot-password';
  const result = await postJson(url, { email });
  const status = result.status;
  const body = result.body;
  try {
    const json = JSON.parse(body);
    const ok = status === 200 && json && (json as any).ok === true;
    if (!ok) {
...
main().catch((e) => {
  console.error('FAIL_MAIL_EX', (e as any)?.message || e);
  process.exit(1);
});
