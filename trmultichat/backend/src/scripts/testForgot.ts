/* Minimal e2e-ish check for forgot-password endpoint */
...
async function main() {
  ...
  try {
    ...
    const ok = status === 200 && json && json.ok === true && typeof json.link === "string";
    if (!ok) {
      console.error("FAIL", { status, json });
      process.exit(1);
    }
    console.log("OK", { status, link: json.link });
    process.exit(0);
  } catch (e) {
    console.error("FAIL_PARSE", { status, body });
    process.exit(1);
  }
}
