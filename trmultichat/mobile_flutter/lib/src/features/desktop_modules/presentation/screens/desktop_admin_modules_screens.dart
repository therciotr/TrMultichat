...
abstract class _BaseCrudScreen<T extends ConsumerStatefulWidget> extends ConsumerState<T> {
...
class _DesktopUsersScreenState extends _BaseCrudScreen<DesktopUsersScreen> {
  ...
  Future<void> fetch() async {
    await safeRun(() async {
      ...
      final data = res.data;
      final list = (data is List ? data : (data is Map && data['users'] is List ? data['users'] : const <dynamic>[]));
      rows = list.whereType<Map>().map((e) => e.cast<String, dynamic>()).toList();
    });
  }
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  controller: password,
                  obscureText: true,
                  decoration: InputDecoration(labelText: id == null ? 'Senha' : 'Nova senha (opcional)'),
                ),
                const SizedBox(height: 8),
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                ...
                if (id == null) {
                  payload['password'] = password.text.trim().isEmpty ? '123456' : password.text.trim();
                  await dio.post('/users', data: payload);
                } else {
                  await dio.put('/users/$id', data: payload);
                  if (password.text.trim().isNotEmpty) {
                    await dio.put('/users/$id/password/raw', data: {'password': password.text.trim()});
                  }
                }
                if (!mounted) return;
                Navigator.pop(ctx);
              ...
              itemBuilder: (_, i) {
                ...
                return Card(
                  child: ListTile(
                    ...
                    subtitle: Text('${r['email'] ?? ''} • perfil: ${r['profile'] ?? 'user'}'),
                    trailing: Wrap(
                      children: [
                        IconButton(onPressed: () => openForm(r), icon: const Icon(Icons.edit_outlined)),
                        IconButton(onPressed: id <= 0 ? null : () => deleteRow(id), icon: const Icon(Icons.delete_outline)),
                      ],
                    ),
                  ),
                );
...
class _DesktopTagsScreenState extends _BaseCrudScreen<DesktopTagsScreen> {
  ...
  Future<void> openForm([Map<String, dynamic>? initial]) async {
    final id = (initial?['id'] as num?)?.toInt();
    final name = TextEditingController(text: (initial?['name'] ?? '').toString());
    final color = TextEditingController(text: (initial?['color'] ?? '#0B4C46').toString());
    await showDialog<void>(
      context: context,
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              TextField(controller: name, decoration: const InputDecoration(labelText: 'Nome')),
              const SizedBox(height: 8),
              TextField(controller: color, decoration: const InputDecoration(labelText: 'Cor (hex)')),
            ],
          ),
        ),
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              final payload = {'name': name.text.trim(), 'color': color.text.trim()};
              if (id == null) {
          ...
          Expanded(
            child: ListView.separated(
              ...
              itemBuilder: (_, i) {
                ...
                return Card(
                  child: ListTile(
                    leading: CircleAvatar(backgroundColor: _parseColor((r['color'] ?? '#0B4C46').toString())),
                    title: Text((r['name'] ?? '').toString(), style: const TextStyle(fontWeight: FontWeight.w800)),
...
class DesktopQueuesScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DesktopQueuesScreen> createState() => _DesktopQueuesScreenState();
}
...
class _DesktopQueuesScreenState extends _BaseCrudScreen<DesktopQueuesScreen> {
  ...
  Future<void> openForm([Map<String, dynamic>? initial]) async {
    ...
    final name = TextEditingController(text: (initial?['name'] ?? '').toString());
    final color = TextEditingController(text: (initial?['color'] ?? '#0B4C46').toString());
    final greeting = TextEditingController(text: (initial?['greetingMessage'] ?? '').toString());
    final outOfHours = TextEditingController(text: (initial?['outOfHoursMessage'] ?? '').toString());
    final orderQueue = TextEditingController(text: (initial?['orderQueue'] ?? '').toString());
    final integrationId = TextEditingController(text: (initial?['integrationId'] ?? '').toString());
    final promptId = TextEditingController(text: (initial?['promptId'] ?? '').toString());
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                TextField(controller: color, decoration: const InputDecoration(labelText: 'Cor (hex)')),
                const SizedBox(height: 8),
                TextField(controller: greeting, decoration: const InputDecoration(labelText: 'Mensagem de saudação')),
                const SizedBox(height: 8),
                TextField(controller: outOfHours, decoration: const InputDecoration(labelText: 'Mensagem fora de horário')),
                const SizedBox(height: 8),
                TextField(controller: orderQueue, decoration: const InputDecoration(labelText: 'Ordem da fila')),
                const SizedBox(height: 8),
                TextField(controller: integrationId, decoration: const InputDecoration(labelText: 'ID integração')),
                const SizedBox(height: 8),
                TextField(controller: promptId, decoration: const InputDecoration(labelText: 'ID prompt')),
              ],
            ),
          ),
        ),
...
class DesktopConnectionsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DesktopConnectionsScreen> createState() => _DesktopConnectionsScreenState();
}

class _DesktopConnectionsScreenState extends _BaseCrudScreen<DesktopConnectionsScreen> {
  List<Map<String, dynamic>> rows = const [];
  @override
  void initState() {
    super.initState();
  ...
  Future<void> openForm([Map<String, dynamic>? initial]) async {
    final id = (initial?['id'] as num?)?.toInt();
    final name = TextEditingController(text: (initial?['name'] ?? '').toString());
    final token = TextEditingController(text: (initial?['token'] ?? '').toString());
    final greeting = TextEditingController(text: (initial?['greetingMessage'] ?? '').toString());
    final completion = TextEditingController(text: (initial?['complationMessage'] ?? initial?['completionMessage'] ?? '').toString());
    final out = TextEditingController(text: (initial?['outOfHoursMessage'] ?? '').toString());
    final rating = TextEditingController(text: (initial?['ratingMessage'] ?? '').toString());
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  TextField(controller: greeting, decoration: const InputDecoration(labelText: 'Mensagem saudação')),
                  const SizedBox(height: 8),
                  TextField(controller: completion, decoration: const InputDecoration(labelText: 'Mensagem conclusão')),
                  const SizedBox(height: 8),
                  TextField(controller: out, decoration: const InputDecoration(labelText: 'Mensagem fora de horário')),
                  const SizedBox(height: 8),
                  TextField(controller: rating, decoration: const InputDecoration(labelText: 'Mensagem avaliação')),
                ],
              ),
            ),
          ),
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              TextField(controller: name, decoration: const InputDecoration(labelText: 'Nome')),
              const SizedBox(height: 8),
              TextField(controller: message, decoration: const InputDecoration(labelText: 'Mensagem'), maxLines: 3),
              const SizedBox(height: 8),
...
class _DesktopPlansScreenState extends _BaseCrudScreen<DesktopPlansScreen> {
  ...
  Future<void> openForm([Map<String, dynamic>? initial]) async {
    final id = (initial?['id'] as num?)?.toInt();
    final name = TextEditingController(text: (initial?['name'] ?? '').toString());
    final users = TextEditingController(text: (initial?['users'] ?? 0).toString());
    final connections = TextEditingController(text: (initial?['connections'] ?? 0).toString());
    final queues = TextEditingController(text: (initial?['queues'] ?? 0).toString());
    final price = TextEditingController(text: (initial?['price'] ?? initial?['value'] ?? 0).toString());
    await showDialog<void>(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              TextField(controller: name, decoration: const InputDecoration(labelText: 'Nome')),
              const SizedBox(height: 8),
              TextField(controller: users, decoration: const InputDecoration(labelText: 'Usuários')),
              const SizedBox(height: 8),
              TextField(controller: connections, decoration: const InputDecoration(labelText: 'Conexões')),
              const SizedBox(height: 8),
              TextField(controller: queues, decoration: const InputDecoration(labelText: 'Filas')),
              const SizedBox(height: 8),
              TextField(controller: price, decoration: const InputDecoration(labelText: 'Preço')),
            ],
          ),
        ),
        actions: [
            ...
            child: ListView.separated(
              ...
              itemBuilder: (_, i) {
                ...
                return Card(
                  child: ListTile(
                    title: Text((r['name'] ?? '').toString(), style: const TextStyle(fontWeight: FontWeight.w800)),
                    subtitle: Text('Usuários: ${r['users'] ?? 0} • Conexões: ${r['connections'] ?? 0} • Valor: ${r['price'] ?? r['value'] ?? 0}'),
                    trailing: Wrap(
                      children: [
                        IconButton(onPressed: () => openForm(r), icon: const Icon(Icons.edit_outlined)),
                        IconButton(
...
class _DesktopHelpsScreenState extends _BaseCrudScreen<DesktopHelpsScreen> {
  ...
  Future<void> openForm([Map<String, dynamic>? initial]) async {
    ...
    final video = TextEditingController(text: (initial?['video'] ?? '').toString());
    final link = TextEditingController(text: (initial?['link'] ?? '').toString());
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              TextField(controller: description, decoration: const InputDecoration(labelText: 'Descrição'), maxLines: 3),
              const SizedBox(height: 8),
              TextField(controller: video, decoration: const InputDecoration(labelText: 'Vídeo')),
              const SizedBox(height: 8),
              TextField(controller: link, decoration: const InputDecoration(labelText: 'Link')),
              const SizedBox(height: 8),
...
class DesktopCampaignsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<DesktopCampaignsScreen> createState() => _DesktopCampaignsScreenState();
}

class _DesktopCampaignsScreenState extends _BaseCrudScreen<DesktopCampaignsScreen> {
  List<Map<String, dynamic>> rows = const [];
  @override
  void initState() {
    super.initState();
  ...
  Future<void> fetch() async {
    await safeRun(() async {
      final res = await dio.get('/campaigns', queryParameters: {'pageNumber': 1});
      final data = (res.data as Map?)?.cast<String, dynamic>() ?? const {};
            ...
            child: rows.isEmpty
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return Card(
                        child: ListTile(
                          title: Text((r['name'] ?? 'Campanha').toString(), style: const TextStyle(fontWeight: FontWeight.w800)),
                          subtitle: Text('Status: ${(r['status'] ?? '-').toString()}'),
                        ),
                      );
                    },
                  ),
